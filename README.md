...
CI builds run on every push to `main` and every pull request (see [.github/workflows/build.yml](.github/workflows/build.yml)). Tagged commits (`v*`) trigger a release build and attach the jar to a GitHub Release. PR commit messages are also linted against the Conventional Commits format (see [.github/workflows/commit-lint.yml](.github/workflows/commit-lint.yml)).

### Commit message format

Subjects must follow [Conventional Commits](https://www.conventionalcommits.org/) — `<type>[(scope)][!]: <subject>` with type in `feat | fix | docs | style | refactor | perf | test | build | ci | chore | revert`. Enable the local pre-commit check once after cloning:
