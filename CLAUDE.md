...
Allowed types: `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`. A `!` after type/scope marks a breaking change.

Enforced both locally (via a `commit-msg` hook in `.githooks/`) and in CI (`.github/workflows/commit-lint.yml`, runs on every push and PR). Enable the local hook once after cloning:

```sh
git config core.hooksPath .githooks
```

Functional commits only — each commit should change behaviour, fix something concrete, or update committed docs. Avoid metadata-only commits.
